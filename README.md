...
| `OAUTH_DB_URL` | PostgreSQL for OAuth refresh tokens and dynamic client registrations (falls back to `AUDIT_DB_URL`). With a database, a client registered through `POST /oauth/register` survives restarts, and registration is idempotent: the same `client_name` and redirect-URI set get the same `client_id` back. Rows are capped at 1000 (least recently seen evicted) and dropped after 90 days unseen; a token exchange or refresh counts as seen. Without one, registrations are in-memory and lost on restart. Every registration is granted scope `mctl`, the only scope | `AUDIT_DB_URL` | No |
...
| `MCTL_USAGE_WRITER_TOKEN` | Bearer token of the usage-writer principal `service:mctl-agents-usage` (mctlhq/.github#50): holds only `usage:write` and may call only `POST /api/v1/usage/records` (see [docs/model-usage-ledger.md](docs/model-usage-ledger.md)). At least 32 characters; a token equal to `MCTL_AGENT_SERVICE_TOKEN` or to a surface token is refused. Unset: only admins can write usage. | — | No |
| `SURFACE_IDENTITY_DB_URL` | PostgreSQL for surface identity links and challenges, and for the canonical principal model (`principals`, `external_identities`; see [docs/principals.md](docs/principals.md)), which mirrors links in the same transaction. Falls back to `AUDIT_DB_URL`; with neither, `/api/v1/surface-identities*` answers 503 and callers carry no principal id. | — | No |
...
| `PRINCIPALS_DISABLED` | Kill switch for the canonical principal model: callers carry no `prn_` id, links are not mirrored, no backfill. Same values as the other kill switches. | unset | No |
