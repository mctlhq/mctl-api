...
Admin-only. Returns workflow_name; poll mctl_get_workflow_status or mctl_list_recent_agent_runs for progress.`),
		mcplib.WithString("service",
			...
			mcplib.Enum("", "mctl-web", "mctl-openclaw", "mctl-docs", "mctl-api", "mctl-portal", "mctl-agent", "mctl-gitops", "mctl-agents", "mctl-telegram", "mctl-design", "mctl-pairdesk", "mctl-academy", "seerrsense", "portfolio", ".github"),
		),
...
Read shepherd_in_loop_known before acting on shepherd_in_loop=false. While status is "Running", that false means either a live execution declining to shepherd or a query that did not complete (an old worker, an outage, a timeout); shepherd_in_loop_known=false is the second — the value is false and the answer is unknown. For a finished execution the false is derived from the status and known is true: a Completed workflow definitively is not ticking a shepherd. For deciding whether the cron should sweep the distinction does not matter; for comparing this answer against anything else it does.
